  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 540 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', alignItems: 'flex-start', gap: 10 }}>
            ...
            <div style={{
              ...
              border:       m.role === 'user' ? '1px solid rgba(59,130,246,0.22)' : '1px solid rgba(255,255,255,0.07)',
              borderRadius: m.role === 'user' ? '14px 14px 4px 14px' : '14px 14px 14px 4px',
              padding: '11px 15px',
              color: '#D0D0D0', fontSize: '0.875rem', lineHeight: 1.65, whiteSpace: 'pre-wrap',
            }}>
              {m.content}
            </div>
          </div>
        ...
        {error && (
          <div style={{ background: 'rgba(224,82,82,0.08)', border: '1px solid rgba(224,82,82,0.2)', borderRadius: 8, padding: '10px 14px', color: '#E05252', fontSize: '0.82rem' }}>
            {error}
          </div>
        )}
        <div ref={bottomRef} />
      ...
      {/* Input */}
      <div style={{ padding: '12px 24px', borderTop: '1px solid #222', display: 'flex', gap: 10 }}>
        <input
          value={input}
          ...
          disabled={loading}
          style={{ flex: 1, background: '#252525', border: '1px solid #2E2E2E', borderRadius: 10, padding: '10px 14px', color: '#F0F0F0', fontSize: '0.875rem', outline: 'none', fontFamily: 'Inter, sans-serif' }}
        />
        <button type="button" onClick={() => send()} disabled={!input.trim() || loading}
          style={{
            padding: '10px 16px', borderRadius: 10, border: 'none',
            background: input.trim() && !loading ? 'rgba(59,130,246,0.2)' : '#1E1E1E',
            color: input.trim() && !loading ? '#3B82F6' : '#333',
